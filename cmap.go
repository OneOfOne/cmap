...
// shardForKey returns the LMap that may contain the specific key.
...
// NewLMap is the equivalent of `m := make(map[interface{}]interface{}, cap)`
...
// You can break early by returning an error or Break.
// It **is** safe to modify the map while using this iterator, however it uses more memory and is slightly slower.
func (lm *LMap) ForEach(keys []interface{}, fn func(key interface{}, val interface{}) error) (err error) {
	lm.l.RLock()
	for key := range lm.m {
...
// You can break early by returning an error or Break.
// It is **NOT* safe to modify the map while using this iterator.
func (lm *LMap) ForEachLocked(fn func(key interface{}, val interface{}) error) (err error) {
	lm.l.RLock()
	defer lm.l.RUnlock()
